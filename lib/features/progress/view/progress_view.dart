  ...
  Widget _buildComparisonChart(AppColors appColors) {
    return BarChart(
      BarChartData(
        ...
        borderData: FlBorderData(show: false),
        barGroups: _weeklyWorkouts.asMap().entries.map((entry) {
          final workoutCal = entry.value.calories;
          final nutritionCal = _weeklyNutrition[entry.key].calories;
          // final netCal = nutritionCal - workoutCal;

          return BarChartGroupData(
            x: entry.key,
            barRods: [
              BarChartRodData(
                toY: workoutCal.toDouble(),
                color: Colors.deepOrange,
                width: 12,
                borderRadius: BorderRadius.circular(3),
              ),
              BarChartRodData(
                toY: nutritionCal.toDouble(),
                color: Colors.green,
                width: 12,
                borderRadius: BorderRadius.circular(3),
              ),
